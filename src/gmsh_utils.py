...
def mesh1(L=1, H=1, order=1, smin=0.01, smax=0.1, dmin=0.1, dmax=0.3):
    ...
    pHoles = [
        gmsh.model.occ.addPoint(3 * L / 4, H / 4, 0),
    ...
    surface = gmsh.model.occ.addPlaneSurface(
        [outer_wire, circle_wire, ellipse_wire, square_wire]
    )
    ...
    gmsh.model.mesh.field.setNumbers(
        min_field, "FieldsList", [threshold_field, backgroundField]
    )
...
def mesh2(L=1, H=1, order=1, smin=0.01, smax=0.1, dmin=0.1, dmax=0.3):
    ...
    gmsh.model.addPhysicalGroup(
        1, [top_curve, bot_curve], tag=1, name="AirfoilBoundary"
    )
