...
def plot_favourable_band(ax, t, c, L_hab, L, H):
    ...
    ax.plot([x0, x0], [0.0, H], linestyle="--", linewidth=1.0, color="forestgreen", alpha=0.9, zorder=7)
    ax.plot([x1, x1], [0.0, H], linestyle="--", linewidth=1.0, color="forestgreen", alpha=0.9, zorder=7)
...
def main(L, H, h, order):
    ...
    parser = argparse.ArgumentParser(
        description="KPP-Fisher species diffusion (Groupe 75 - LEPL1110)")
    parser.add_argument("-order",   type=int,   default=order)
    parser.add_argument("--theta",  type=float, default=1.0,    help="θ-scheme (1=implicit Euler)")
    ...
    (elemType, nodeTags, nodeCoords,
     elemTags, elemNodeTags, bnds, bnds_tags) = build_2d_rectangle_mesh(
         L=L, H=H, size_field=size_field, order=order)
    ...
    # ------------------------------------------------------------------
    U = np.array([u_init(x, x0, u0_max, sigma) for x in dof_coords],
                 dtype=float)
    ...
    for step in range(nstep):
        ...
        f_np1 = make_explicit_source(
            U, dof_coords, tag_to_dof, t + dt,
            c, L_hab, r, r_tilde, K)

        # --- Assemble stiffness + RHS at t_n and t_{n+1} ---
        K_lil_n,   F_n   = assemble_stiffness_and_rhs(
            elemTags, elemNodeTags, jac, det, coords,
            w, N, gN, kappa, f_n, tag_to_dof)

        K_lil_np1, F_np1 = assemble_stiffness_and_rhs(
            elemTags, elemNodeTags, jac, det, coords,
            w, N, gN, kappa, f_np1, tag_to_dof)
